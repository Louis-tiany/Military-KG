...
export default {
      ...
      menu: [
        ...
        },
        {
          ...
          action: (ele, d) => {
            console.log(d.id)
            this.$axios({
              url: 'http://localhost:8000/searchOneNode',
              method: 'POST',
              header: {
              ...
              data: {
                ...
                for(let i = 0; i < response.data['nodes'].length; ++i) {
                  if(nodeSet.indexOf(response.data['nodes'][i]['id']) === -1) {
                    nodeSet.push(response.data['nodes'][i]["id"])
                    nodes.push(response.data['nodes'][i])
                  }
                }
                console.log(nodes)

                // for(let i = 0; i < nodes.length; ++i) {
                //   if(nodeSet.indexOf(nodes[i]["id"]) == -1) {
                //     nodeSet.push(nodes[i]["id"])
                //     this.nodes.push({
                //       id: nodes[i]["id"],
                //       label: nodes[i]["label"],
  ...
  methods: {
    ...
    d3init () {
      ...
      console.log(this.data.nodes[0].label)
      for (let i = 0; i < this.data.nodes.length; ++i) {
        this.labels.push(this.data.nodes[i].label)
      }
